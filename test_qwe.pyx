...
def run_main(color_num):
    ...
    #vertices = readfile('graph_real.txt')
    ...
    for i in range(pop_size):
        ...
        print(element["cost"])
        start = time.time()
        num_of_vdls_swaps += vdls(element, vertices)
        num_of_vdls += 1
        elapsed = (time.time() - start)
    ...
    while True:
        ...
        while i < pop_size:
            ...
            num_of_vdls_swaps += vdls(offspring, vertices)
            num_of_vdls += 1
        ...
        run_number += 1
        time_end_cpu = (time.process_time() - time_start_cpu)
        measures.append({"run_num": run_number, "score": best_score, "elapsed_time": elapsed_time_loop, "cpu_time": time_end_cpu})
    ...
    thefile = open(str(K) + 'pyx.txt', 'w')



    thefile.write("vdls_count %s\n" % num_of_vdls)
    thefile.write("vdls_swaps %s\n" % num_of_vdls_swaps)
    ...
    thefile.write("%s " % str(best1["cost"]))
    thefile.write("%s\n" %  best1["elements"])

    thefile.write("----------------")

    thefile.write("%s " % str(best2["cost"]))
    thefile.write("%s\n" %  best2["elements"])
